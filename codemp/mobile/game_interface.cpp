// OpenJK multiplayer (codemp) mobile entry point.
//
// Bridges the OpenTouch Android touch layer (Clibs_OpenTouch/game_interface.h)
// to the OpenJK MP engine. The touch UI / JNI bridge calls these Portable*
// functions to drive input, query screen state and pump the engine.
//
// This mirrors the single-player code/mobile/game_interface.cpp; the differences
// are MP-specific: the MP client has no CL_IsRunningInGameCinematic() and no
// game-side force-power query hook, so the screen-mode logic uses only the
// connection state and the force-power query is left as a stub (returns false).
//
...
// OpenJK MP client state + command buffer + usercmd_t. client.h resolves its own
// relative includes from codemp/client/, so it is safe to pull in from here.
...
// SDL's internal keyboard injection (same approach iortcw / TFE / SP use): pushes
// a key event into SDL's queue under SDL's lock. OpenJK reads it through the normal
...
// (engine thread) ring buffer; same lock-free pattern the SP/jk3 ports used.
...
// Look accumulators. Written on the touch thread, drained on the engine thread in
// CL_AndroidMove() by forwarding to MouseMove().
...
// commands. In menus, navigation actions are injected as SDL keys instead so the
// UI behaves like a real keyboard/mouse.
...
// Map OpenJK MP's client state onto the touch screen mode so the right control
// set shows: console overlay -> TS_CONSOLE, any UI/menu up -> TS_MENU, a cinematic
// -> TS_BLANK (full-screen pass-through layer whose tap sends Enter to skip it),
// live gameplay -> TS_GAME. Anything else (loading, disconnected at the main menu)
